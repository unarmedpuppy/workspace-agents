 ...
 * - No agents/ directory → Run scaffold-workspace
 * - Existing agents/ directory → Run upgrade-workspace
 */
...
async function autoSetup(projectRoot = process.cwd()) {
  ...
  if (action === 'scaffold') {
    ...
    console.log(chalk.cyan('→ Running scaffold-workspace skill...\n'));
    ...
    try {
      ...
      console.error(chalk.white('  node agents/skills/scaffold-workspace/scripts/scaffold.js'));
      process.exit(1);
    }
  } else {
    console.log(chalk.green('✓ Existing agents/ directory detected'));
    console.log(chalk.cyan('→ Running upgrade-workspace skill...\n'));
    ...
    try {
      ...
      if (fs.existsSync(upgradeSkillPath)) {
        ...
        console.error(chalk.red('❌ upgrade-workspace skill not found!'));
        console.error(chalk.yellow('\nThe upgrade skill is not yet implemented.'));
        console.error(chalk.yellow('Expected location: agents/skills/upgrade-workspace/scripts/upgrade.js'));
        console.error(chalk.white('\nTo scaffold in a clean directory:'));
        console.error(chalk.white('  1. Move or backup existing agents/ directory'));
        console.error(chalk.white('  2. Run: node agents/skills/scaffold-workspace/scripts/scaffold.js'));
        process.exit(1);
      }
    } catch (error) {
      console.error(chalk.red('\n❌ Upgrade failed:'), error.message);
      console.error(chalk.yellow('\nYou can run upgrade manually when implemented:'));
      console.error(chalk.white('  node agents/skills/upgrade-workspace/scripts/upgrade.js'));
      process.exit(1);
    }
  }
}
